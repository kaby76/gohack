...
func (d *DFA) getState(v *DFAState) (*DFAState, bool) {
	s, ok := d.states.Get(v)
	return s, ok
}

func (d *DFA) setStates(states *HashTable) {
	d.states = states
}

func (d *DFA) setState(key *DFAState, state *DFAState) {
...
func (d *DFA) sortedStates() []*DFAState {
	vs := make([]*DFAState, 0, d.states.Len())

	for v := range d.states.MyIterator() {
		vs = append(vs, v)
	}
